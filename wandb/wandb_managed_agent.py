...
def agent_consume_sandbox(
    sweep_id: str,
    entity: str,
    project: str,
    artifact_id: str,
) -> ManagedAgent:
    """Return a :class:`ManagedAgent` for decorating a sandbox before starting it.

    Args:
        sweep_id: W&B sweep ID (short ID, ``"project/id"``, or
            ``"entity/project/id"``).
        entity: W&B entity (username or team name).
        project: W&B project name.
        artifact_id: Fully-qualified artifact identifier, e.g.
            ``"entity/project/name:latest"``.

    Returns:
        A :class:`ManagedAgent`.  Call ``.consume_sandbox(sandbox)`` to
        configure a sandbox, then start it with ``sandbox.start()``.
    ...
    Example::
        ...
        with Session() as session:
            ...
            sandbox.start()   # downloads artifact, starts wandb agent
    """
    return ManagedAgent(
        sweep_id=sweep_id,
        entity=entity,
